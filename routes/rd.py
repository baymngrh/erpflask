...
def get_rd_analytics():
    ...
    try:
        ...
        # Experiment success trends
        experiment_trends = db.session.query(
        ...
        ).filter(ResearchProject.budget > 0).all()
        ...
        return jsonify({
            ...
            'experiment_success_trends': [
                {
                    'month': t.month,
                    'total': t.total,
                    'successful': t.successful,
                    'success_rate': (t.successful / max(1, t.total)) * 100
                }
                for t in experiment_trends
            ],
            ...
            'budget_utilization': [
                {
                    ...
                    'utilization_rate': (float(b.actual_cost) / float(b.budget)) * 100
                }
