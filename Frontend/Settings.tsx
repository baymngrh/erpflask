 ...
 <Worksheet ss:Name="Inventory">
  ...
  const handleDeleteUser = async (userId: number, userName: string) => {
    ...
    try {
      ...
      showMessage('error', `Failed to deactivate user: ${error.data?.error || error.message}`);
    } finally {
      setIsLoading(false);
    }
  };
      ...
      return (
        <div className="text-center py-8">
          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            ...
            {usersError && (
              <div className="mt-2 p-2 bg-red-100 rounded text-xs text-red-700">
                Users Error: {JSON.stringify(usersError)}
              </div>
            )}
          </div>
        </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="mt-8 pt-6 border-t flex justify-end space-x-4">
              ...
              <button
                ...
                disabled={isLoading || (activeTab === 'backup') || (activeTab === 'notifications') || (activeTab === 'users')}
