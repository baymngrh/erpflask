        ...
        <div className="card p-6">
          ...
          <div className="space-y-4">
            ...
            <div className="border-b pb-4">
              ...
              <div className="ml-4 space-y-1">
                <div className="flex justify-between">
                  ...
                  <span>{formatRupiah(incomeStatement.revenue.sales_revenue)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Other Income</span>
                  <span>{formatRupiah(incomeStatement.revenue.other_income)}</span>
                </div>
                <div className="flex justify-between font-semibold border-t pt-1">
                  <span>Total Revenue</span>
                  <span>{formatRupiah(incomeStatement.revenue.total_revenue)}</span>
                </div>
              </div>
            </div>
            ...
            <div className="border-b pb-4">
              ...
              <div className="ml-4 space-y-1">
                <div className="flex justify-between">
                  ...
                  <span>{formatRupiah(incomeStatement.cost_of_sales.direct_materials)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Direct Labor</span>
                  <span>{formatRupiah(incomeStatement.cost_of_sales.direct_labor)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Manufacturing Overhead</span>
                  <span>{formatRupiah(incomeStatement.cost_of_sales.manufacturing_overhead)}</span>
                </div>
                <div className="flex justify-between font-semibold border-t pt-1">
                  <span>Total Cost of Sales</span>
                  <span>({formatRupiah(incomeStatement.cost_of_sales.total_cogs)})</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-between font-bold text-lg bg-blue-50 p-2 rounded">
              ...
              <span className="text-blue-600">{formatRupiah(incomeStatement.gross_profit)}</span>
            </div>
            ...
            <div className="border-b pb-4">
              ...
              <div className="ml-4 space-y-1">
                <div className="flex justify-between">
                  ...
                  <span>{formatRupiah(incomeStatement.operating_expenses.selling_expenses)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Administrative Expenses</span>
                  <span>{formatRupiah(incomeStatement.operating_expenses.administrative_expenses)}</span>
                </div>
                <div className="flex justify-between font-semibold border-t pt-1">
                  <span>Total Operating Expenses</span>
                  <span>({formatRupiah(incomeStatement.operating_expenses.total_operating_expenses)})</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-between font-bold text-lg bg-green-50 p-2 rounded">
              ...
              <span className="text-green-600">{formatRupiah(incomeStatement.operating_income)}</span>
            </div>
            ...
            <div className="border-t pt-4">
              <div className="flex justify-between">
                ...
                <span>({formatRupiah(incomeStatement.income_tax)})</span>
              </div>
              <div className="flex justify-between font-bold text-xl bg-indigo-50 p-3 rounded mt-2">
                <span>NET INCOME</span>
                <span className="text-indigo-600">{formatRupiah(incomeStatement.net_income)}</span>
              </div>
            </div>
          </div>
        </div>
              ...
              try {
                ...
                if (activeReport === 'income') {
                  reportData = incomeStatement ? [
                    { Item: 'Sales Revenue', Amount: incomeStatement.revenue.sales_revenue },
                    { Item: 'Other Income', Amount: incomeStatement.revenue.other_income },
                    { Item: 'Total Revenue', Amount: incomeStatement.revenue.total_revenue },
                    { Item: 'Direct Materials', Amount: -incomeStatement.cost_of_sales.direct_materials },
                    { Item: 'Direct Labor', Amount: -incomeStatement.cost_of_sales.direct_labor },
                    { Item: 'Manufacturing Overhead', Amount: -incomeStatement.cost_of_sales.overhead },
                    { Item: 'Gross Profit', Amount: incomeStatement.gross_profit },
                    { Item: 'Operating Expenses', Amount: -incomeStatement.operating_expenses.total },
                    { Item: 'Net Income', Amount: incomeStatement.net_income }
                  ] : []
                } else if (activeReport === 'balance') {
                  reportData = balanceSheet ? [
                    { Category: 'Current Assets', Item: 'Cash', Amount: balanceSheet.assets.current.cash },
                    { Category: 'Current Assets', Item: 'Accounts Receivable', Amount: balanceSheet.assets.current.accounts_receivable },
                    { Category: 'Current Assets', Item: 'Inventory', Amount: balanceSheet.assets.current.inventory },
                    { Category: 'Fixed Assets', Item: 'Property Plant Equipment', Amount: balanceSheet.assets.fixed.property_plant_equipment },
                    { Category: 'Current Liabilities', Item: 'Accounts Payable', Amount: balanceSheet.liabilities.current.accounts_payable },
                    { Category: 'Current Liabilities', Item: 'Short Term Debt', Amount: balanceSheet.liabilities.current.short_term_debt },
                    { Category: 'Equity', Item: 'Share Capital', Amount: balanceSheet.equity.share_capital },
                    { Category: 'Equity', Item: 'Retained Earnings', Amount: balanceSheet.equity.retained_earnings }
                  ] : []
                } else {
                  reportData = cashFlow ? [
                    { Activity: 'Operating Activities', Item: 'Net Income', Amount: cashFlow.operating.net_income },
                    { Activity: 'Operating Activities', Item: 'Depreciation', Amount: cashFlow.operating.depreciation },
                    { Activity: 'Operating Activities', Item: 'Working Capital Changes', Amount: cashFlow.operating.working_capital_changes },
                    { Activity: 'Investing Activities', Item: 'Capital Expenditures', Amount: cashFlow.investing.capital_expenditures },
                    { Activity: 'Financing Activities', Item: 'Debt Proceeds', Amount: cashFlow.financing.debt_proceeds },
                    { Activity: 'Financing Activities', Item: 'Dividends Paid', Amount: cashFlow.financing.dividends_paid }
                  ] : []
                }
